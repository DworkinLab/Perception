...
ggplot(df, aes(y=Female_Int, x=reorder(DGRP, Female_Int))) + geom_linerange(aes(ymin=f.low, ymax=f.high), colour="black") + 
  geom_point( colour="red") + 
  coord_flip() + 
  labs(y="Intercept", x="DGRP Females")

ggplot(df, aes(y=Male_Int, x=reorder(DGRP, Male_Int))) + geom_linerange(aes(ymin=m.low, ymax=m.high), colour="black") + 
...
#Large model #1: Sex and DGRP variances
mod1 <- glmer(cbind(Spider, Not_spider) ~ 1 + Sex + (1|Date) + (0 + Sex|DGRP),
              data = DGRP_by_counts, family = "binomial")
summary(mod1)

#DGRP Variances (small model of above, large model for Mod3)
mod2 <- glmer(cbind(Spider, Not_spider) ~ 1 + Sex + (1|Date) + (1|DGRP),
              data = DGRP_by_counts, family = "binomial")
summary(mod2)

#Small Model: no DGRP or Sex variances randef
